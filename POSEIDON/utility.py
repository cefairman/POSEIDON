...
def read_data_mod(data_dir, fname, wl_unit='micron', bin_width='half', data_type=None,
                  spectrum_unit='(Rp/Rs)^2', skiprows=None):
    '''
...
def generate_latex_param_names(param_names):
    ...
    for param in param_names:
        ...
        captured_characters = np.zeros(len(param)).astype(
            np.int32)  # Stays zero for entries with solo letters (e.g. 'H' in 'H2O')
...
def write_MultiNest_results_mod(planet, model, data, retrieval_name,
    ...
    if data_type != 'GMM':
        ...
        # not priority to calculate straight away. Will come back once retrieval is working
        print('norm log and chisq not currently calculated for GMM')
        norm_log = 'none'
