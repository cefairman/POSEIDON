...
def TRIDENT(P, r, r_up, r_low, dr, wl, kappa_clear, kappa_cloud,
            b_p, y_p, R_s,
            phi_grid, dphi_grid, phi_edge, theta_grid, theta_edge_all, N_sectors, N_zones,
    ...
    @char: CHANGES MADE:
           Removed extend_rad_transfer_grids
           Output of this function has been introduced into the input of TRIDENT.

           Mayy be able to get rid of j_sector_back as we don't have a background and cloud atmosphere.



            Arguments have been removed where they are no longer used (marked in the args: list as a *)

            Not considering cases where the planet partially overlaps the star (only full transits)
            Can introduce partial transits later: the partial transits just splits the atmosphere into a higher
            resolution, where N_phi_max = 36, we already define this high resolution.
    Args:
        P (np.array of float):
 ...
 #   print("Beta_max = " + str(beta_max))
    ...
    # @char: removed this function and put 3D output into input of TRIDENT.
    ...
    # @char_start: adding in variables from extend_rad_transfer_grids

    # Define array specifying which zones contain clouds (0 -> clear, 1-> cloudy)
    # all cloudy_zones, cloudy_sectors = 0, no clouds in GCM
    cloudy_zones = np.zeros(N_zones).astype(np.int64)
    ...
    for k in range(N_zones):
        # Find the zone in which this angle lies, store for later referencing in radiative transfer
        k_zone_back[k] = prior_index(theta_grid[k], theta_edge_all, 0)  # Background atmosphere only
    ...
    for j in range(N_phi):
        ...
        # Find the terminator sector in which this angle lies
        j_sector_in = prior_index(phi_grid[j], phi_edge, 0)  # All sectors (including clouds)
        j_sector_back_in = prior_index(phi_grid[j], phi_edge, 0)  # Background atmosphere only
