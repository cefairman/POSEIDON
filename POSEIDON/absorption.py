...
def refractive_index(wl, n_ref, species):
    ...
    if (species in ['H2', 'O2', 'N2O', 'CO2', 'NH3']):
        ...
        alpha = ((1.0 / 3.0) * ((f_par / (w_par_sq - (nu / 219474.6305) ** 2)) +  # Polarisability - Hohm, 1993
                                2.0 * (f_perp / (
                        w_perp_sq - (nu / 219474.6305) ** 2)))) * 0.148184e-24  # Convert to cm^3

        eta = np.sqrt((1.0 + (8.0 * np.pi * n_ref * alpha / 3.0)) / (
                1.0 - (4.0 * np.pi * n_ref * alpha / 3.0)))  # Lorentz-Lorenz relation

    # Many of these below are from: http://refractiveindex.info (Polyanskiy, 2016)
    elif (species == 'He'):

        eta = 1.0 + ((0.014755297 / (426.29740 - 1.0 / (
                wl ** 2))) * 1.0018141444038913)  # Cuthbertson & Cuthbertson, 1936 (multiplicative factor for continuity)
        ...
        eta = 1.0 + ((5677.465e-8 + (318.81874e4 / (
                14.4e9 - nu ** 2))) * 1.0001468057477378)  # Sneep & Ubachs, 2005 (sec 4.2) / Bates, 1984  (fmultiplicative actor for continuity)
        eta[wl < 0.2540] = 1.00030493
        eta[wl > 0.46816] = 1.0 + (6498.2e-8 + (307.43305e4 / (
                14.4e9 - nu[wl > 0.46816] ** 2)))  # Sneep & Ubachs, 2005 (sec 4.2) / Peck & Khanna, 1966
        ...
        eta = 1.0 + ((3.011e-2 / (124.40 - 1.0 / (wl ** 2))) +  # Hill & Lawrence, 1986
                     (7.46e-3 * (0.203 - 1.0 / wl)) / (
                             1.03 - 1.98e3 / (wl ** 2) + 8.1e4 / (wl ** 4) - 1.7e8 / (wl ** 8)))
...
def King_correction(wl, species):
    ...
    if (species in ['H2', 'O2', 'N2O', 'CO2', 'NH3']):
        ...
        F = 1.000000 * nu ** 0  # Spherical atom, so King correction factor = 1
    elif (species == 'H2O'):
        F = 1.001005 * nu ** 0  # Derived from values in Hinchliffe , 2007
    elif (species == 'CH4'):
        F = 1.000000 * nu ** 0  # Negligible difference from unity - Sneep & Ubachs, (sec 5.2)
    elif (species == 'O3'):
        F = 1.060000 * nu ** 0  # Brasseur & De Rudder, 1986
    elif (species == 'CO'):
        F = 1.016995 * nu ** 0  # Mean value from Bogaard+, 1978
    elif (species == 'C2H2'):
        F = 1.064385 * nu ** 0  # Mean value from Bogaard+, 1978
    elif (species == 'C2H4'):
        F = 1.042043 * nu ** 0  # Mean value from Bogaard+, 1978
    elif (species == 'C2H6'):
        F = 1.006063 * nu ** 0  # Mean value from Bogaard+, 1978
    elif (species == 'OCS'):
        F = 1.138786 * nu ** 0  # Mean value from Bogaard+, 1978
    elif (species == 'CH3Cl'):
        F = 1.026042 * nu ** 0  # Mean value from Bogaard+, 1978
    elif (species == 'H2S'):
        F = 1.001880 * nu ** 0  # Mean value from Bogaard+, 1978
    elif (species == 'SO2'):
        F = 1.062638 * nu ** 0  # Mean value from Bogaard+, 1978

    else:
        F = 1.0 * nu ** 0  # If no data available, set King correction factor to 1.0
...
def Rayleigh_cross_section(wl, species):
    ...
    n_ref = (101325.0 / (
            sc.k * 273.15)) * 1.0e-6  # Number density (cm^-3) at 0 C and 1 atm (1.01325 bar) - http://refractiveindex.info
...
def opacity_tables(rank, comm, wl_model, chemical_species, active_species,
    ...
    cia_stored = shared_memory_array(node_rank, node_comm,
                                     (N_cia_pairs, N_T_fine, N_wl))  # Collision-induced absorption
    ...
    sigma_stored = shared_memory_array(node_rank, node_comm,
                                       (N_species_active, N_P_fine, N_T_fine, N_wl))  # Molecular and atomic opacities
    ...
    if (node_rank == 0):
        ...
        # For all other applications
            ...
            log_P_grid = np.array(
                opac_file['H2O/log(P)'])  # Units: log10(P/bar) - H2O choice arbitrary, all P grids are the same
        ...
        if (testing == False):  # The automated tests don't download the full cross sections
            ...
            for q in range(N_species_active):
                ...
                # Evaluate temperature interpolation weighting factor
                y_q = np.zeros(N_T_fine,
                               dtype=np.int64)  # Index of T in cross section arrays prior to fine temperature value
        ...
        for k in range(N_zones):
            ...
            if (enable_haze == 1):
                ...
                for i in range(i_bot, N_layers):
                    ...
                    haze_amp = (n[
                                    i, j, k] * a * 5.31e-31)  # Final factor is H2 Rayleigh scattering cross section at 350 nm
                    ...
                    for aerosol in range(len(n_aerosol_array)):
                        for i in range(i_bot, N_layers):
                            for q in range(len(wl)):
                                kappa_cloud[i, j, k, q] += n_aerosol_array[aerosol][i, j, k] * sigma_Mie_array[aerosol][
                                    q]
                ...
                # Opaque Deck is the first element in n_aerosol_array
                    ...
                    for aerosol in range(len(n_aerosol_array)):
                        if aerosol == 0:
                            ...
                            for i in range(i_bot, N_layers):
                                for q in range(len(wl)):
                                    kappa_cloud[i, j, k, q] += n_aerosol_array[aerosol][i, j, k] * \
                                                               sigma_Mie_array[aerosol - 1][q]
...
def interpolate_sigma_LBL(log_sigma, nu_model, nu_opac, P, T, log_P_grid, T_grid,
    ...
    z_grid_min = closest_index(nu_model_min, nu_opac[0], nu_opac[-1],
                               N_nu_opac)  # Minimum opacity grid index to consider
    z_grid_max = closest_index(nu_model_max, nu_opac[0], nu_opac[-1],
                               N_nu_opac)  # Maximum opacity grid index to consider
        ...
        for k in range(N_nu):  # Loop over model wavenumbers
            ...
            if ((nu_model_k < nu_opac_min) or (nu_model_k > nu_opac_max)):
                ...
                if (x[i] == -1):
                    ...
                    # If layer T above maximum on opacity grid (3500 K), set value to edge case
                        ...
                        sigma_inp[i, ((N_wl - 1) - k)] = 10 ** (
                            reduced_log_sigma[0, (N_T - 1), k])  # No interpolation needed
                ...
                # If layer P above maximum on opacity grid (100 bar), set value to edge case
                    ...
                    if (y[i] == -1):
                        sigma_inp[i, ((N_wl - 1) - k)] = 10 ** (
                            reduced_log_sigma[(N_P - 1), 0, k])  # No interpolation needed

                    # If layer T also above maximum on opacity grid (3500 K), set value to edge case
                    elif (y[i] == -2):
                        sigma_inp[i, ((N_wl - 1) - k)] = 10 ** (
                            reduced_log_sigma[(N_P - 1), (N_T - 1), k])  # No interpolation needed
    ...
    for j in range(N_sectors):
        ...
        for k in range(N_zones):
            ...
            for q in range(N_cia_pairs):
                ...
                # Evaluate temperature interpolation weighting factor
                y_cia_q = np.zeros(N_layers,
                                   dtype=np.int64)  # Layer idex in cia arrays prior to layer temperature value
            ...
            for q in range(N_species_active):
                ...
                # Evaluate temperature interpolation weighting factor
                y_q = np.zeros(N_layers,
                               dtype=np.int64)  # Layer index in cross section arrays prior to layer temperature value
...
def extinction_spectrum_pressure_contribution(chemical_species, active_species, cia_pairs, ff_pairs, bf_species,
                                              ...
                                              bf_stored, enable_haze, enable_deck, enable_surface, N_sectors,
                                              N_zones, T_fine, log_P_fine, P_surf, P_deep=1000.0,
                                              contribution_molecule='', layer_to_ignore=0,
                                              total=False):  # DOES P_DEEP SOLVE BD PROBLEM?!
